...
module CceReportMod
  ...
  module InstanceMethods
    ...
    def batch_in_context
      @batch_in_context = Batch.find_by(id: batch_in_context_id)
    end

    def batch_in_context=(arg)
      if arg.instance_of?(Batch) || arg.nil?
    ...
    def subject_fa_scores
      hsh = {}
      cce_reports.scholastic.all(select: "cce_reports.*,exams.subject_id,fa_criterias.fa_group_id",
                                 joins: 'INNER JOIN fa_criterias ON cce_reports.observable_id = fa_criterias.id INNER JOIN exams ON exams.id = cce_reports.exam_id', conditions: ["batch_id=?", batch_in_context_id]).group_by(&:subject_id).each do |key, val|
        hsh[key.to_i] = {}
        val.group_by(&:exam_id).each do |e_id, e_val|
          hsh[key.to_i][e_id] = {}
          e_val.group_by(&:fa_group_id).each do |k, v|
            hsh[key.to_i][e_id][k.to_i] = (if v.count.positive?
                                             (v.sum do |e|
                                                e.grade_string.to_f
                                              end / v.count)
                                           else
    ...
    def make_scholastic_report
      ...
      all_weightages = CceWeightage.all(joins: :courses,
                                        conditions: { courses: { id: batch_in_context.course_id } })
      sub_fa_scores.each do |_k1, v1|
        v1.each { |_k2, v2| fg_ids << v2.keys }
        exam_ids << v1.keys
      end
      examscores = exam_scores.all(joins: { exam: :exam_group },
                                   conditions: { exam_id: exam_ids.flatten.uniq, exam_groups: { batch_id: batch_in_context_id } }, include: { exam: :exam_group })
      ...
      sub_fa_scores.each do |subject_id, subval|
        ...
        subval.each do |exam_id, examval|
          ...
          examval.each do |fg_id, score|
            ...
            se.fa[fg_id] = score * fg.max_marks / max_credit_point unless fg.nil?
          end
          ...
          if examscore
            ...
            fa_weight = all_weightages.find do |w|
              w.cce_exam_category_id == (examscore.exam.exam_group.cce_exam_category_id || 1) and w.criteria_type == "FA"
            end
            sa_weight = all_weightages.find do |w|
              w.cce_exam_category_id == (examscore.exam.exam_group.cce_exam_category_id || 1) and w.criteria_type == "SA"
            end
    ...
    def coscholastic_scores
      hsh = {}
      cce_reports.coscholastic.all(
        select: "cce_reports.*,observations.observation_group_id,observations.name AS o_name, observations.sort_order ", joins: 'INNER JOIN observations ON cce_reports.observable_id = observations.id', conditions: [
          "batch_id=?", batch_in_context_id
        ], order: "observations.sort_order ASC"
      ).group_by(&:observation_group_id).each do |key, val|
        hsh[key.to_i] = {}
        val.group_by(&:observable_id).each do |k, v|
          hsh[key.to_i][k] = {}
          hsh[key.to_i][k][:grade] = v.find(&:grade_string).try(:grade_string)
          hsh[key.to_i][k][:observation_name] = v.find(&:grade_string).try(:o_name)
          hsh[key.to_i][k][:sort_order] = v.find(&:grade_string).try(:sort_order)
        end
      end
      hsh
    end
    ...
    def cce_report_cache_key
      "cce_report/batch/#{batch_in_context_id}/#{self.class.name}/#{id}"
    end
  end
end
