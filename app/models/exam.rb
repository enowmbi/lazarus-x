...
class Exam < ApplicationRecord
  ...
  validates :maximum_marks, :minimum_marks, numericality: { allow_nil: true }
  validates :maximum_marks, :minimum_marks, presence: { if: :validation_should_present?, on: :update }
  ...
  def validate
    errors.add_to_base(I18n.t('minmarks_cant_be_more_than_maxmarks').to_s) \
      if minimum_marks && maximum_marks && (minimum_marks > maximum_marks)
        errors.add_to_base(I18n.t('minmarks_cant_be_more_than_maxmarks').to_s) \
      if minimum_marks && maximum_marks && (minimum_marks > maximum_marks)
      if !(start_time.nil? || end_time.nil?) && (end_time < start_time)
        errors.add_to_base(I18n.t('end_time_cannot_before_start_time').to_s)
    end
  end
  ...
  def score_for(student_id)
    exam_score = exam_scores.where(student_id: student_id).first
    exam_score.nil? ? ExamScore.new : exam_score
  end

  def class_average_marks
    results = ExamScore.where(exam_id: self)
    scores = results.collect { |x| x.marks unless x.marks.nil? }
    scores.delete(nil)
    return (scores.sum / scores.size) unless scores.size.zero?
  ...
  def update_exam_group_date
    ...
    if !group.exam_date.nil? && (start_time.to_date < group.exam_date)
      group.update(:exam_date, start_time.to_date)
    end
  end
  ...
  def create_exam_event
    if event.blank?
      ...
      BatchEvent.create do |be|
        be.event_id = new_event.id
        be.batch_id = exam_group.batch_id
      end
      # self.event_id = new_event.id
      update(event_id: new_event.id)
    end
  end

  def update_exam_event
    event.update(start_date: start_time, end_date: end_time) if event.present?
  end
end
