...
class EmployeesSubject < ApplicationRecord
  belongs_to :employee
  belongs_to :subject
  ...
  def self.allot_work(employee_subj_ids)
    ...
    transaction do
      ...
      employee_subj_ids.each do |subj_id, emp_id|
        a = find_or_create_by_subject_id(subject_id: subj_id)
        a.employee_id = emp_id
        a.save
        emp_subjs << a
      end

      employee_assignments = emp_subjs.group_by(&:employee_id)
      emp_assigned_hours = {}
      ...
      overloaded_emps = emp_assigned_hours.reject { |_emp_id, details| details[:assigned].to_i <= details[:max].to_i }
      status = overloaded_emps.blank?

      if overloaded_emps.present?
        overloaded_emps.each do |_emp_id, details|
          error_carrier.errors.add_to_base("#{details[:emp_name]} has #{details[:assigned] - details[:max].to_i} extra periods assigned")
        end
        #        raise ActiveRecord::Rollback
      end
    end
    [status, error_carrier]
  end
end
